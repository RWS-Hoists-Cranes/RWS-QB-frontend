...
function CreateInvoicesContent() {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      ...
      try {
        ...
        setLineItems(
          lines.slice(0, lines.length - 1).map((lineItem) => {
            return {
  ...
  const handleCreateInvoice = async () => {
    ...
    try {
      const invoiceData = await constructInvoice(
        order.estimate.CustomerRef.value,
        invoiceNo,
        lineItems,
        estimateId
      );
      ...
      toast({
        title: "Error",
        description:
          "Failed to create invoice: Ensure that invoice number is unique.",
        variant: "destructive",
      });
    }
  };
    ...
    <Card className="w-full max-w-7xl mx-auto mt-8">
      ...
      <CardContent>
        ...
        <div className="mt-6 flex justify-end space-x-4">
          <Button
            variant="outline"
            onClick={() => router.push("/displayestimates")}
          >
            Cancel
          </Button>
